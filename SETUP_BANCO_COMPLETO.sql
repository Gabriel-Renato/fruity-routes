-- ============================================================================
-- SETUP COMPLETO DO BANCO - StarFruitC
...
-- carts e cart_items (carrinho de compras)
...
begin
  ...
  where table_schema = 'public' and table_name in ('profiles', 'stores', 'products', 'categories', 'product_categories', 'addresses', 'carts', 'cart_items', 'orders', 'order_items');
  ...
  raise notice 'Tabelas criadas: %', v_table_count;
  raise notice 'Funções criadas: %', v_function_count;
  raise notice 'Triggers criados: %', v_trigger_count;
  
  if v_table_count >= 9 and v_function_count >= 2 and v_trigger_count >= 2 then
    raise notice 'Banco de dados configurado com sucesso!';
  else
    raise warning 'Alguns objetos nao foram criados corretamente';
  end if;
end $$;
